...
class EavEntityBehavior extends ModelBehavior{
        ...
        function setup(&$Model, $settings = array())
        {
                ...
                {
                		if(isset($Model->hasMany[$assoc]['isEav']))
                		{
        ...
        {  	
        	...
        	foreach($results as $result)
        	{
        		...
        		{
        			$attributes = array();
					if(!empty($result[$assoc['with']]))
					{						
						foreach($result[$assoc['with']] as $value)
						{
							$attributes[] = $eavAttribute->find('all', array('conditions' => array('attribute_id =' => $value['attribute_id'])));
